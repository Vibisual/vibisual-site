...
쿠키도 광고 추적기도 쓰지 않습니다. 브라우저에 남는 것은 고른 언어(`vibisual-lang`)와
이미 셌는지 표시(`vibisual-visited`) 둘뿐입니다.

바깥으로 나가는 요청은 셋입니다 — 위 unpkg 세 개, GitHub 공개 API(별 수 · 릴리스
다운로드 수), 그리고 방문자 수 카운터(`abacus.jasoncameron.dev`)입니다. 카운터는
처음 온 브라우저에서만 `/hit` 으로 1 을 올리고 그다음부터는 `/get` 으로 읽기만 하므로
조회수가 아니라 사람 수에 가깝습니다. 키가 없는 공개 카운터라 누구나 `/hit` 을 호출해
숫자를 올릴 수 있다는 점은 알고 쓰는 것입니다.

페이지 하단에 같은 내용을 12개 언어로 적어 두었습니다.
